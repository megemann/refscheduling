class Scheduler:
    ...
    def optimize(self):
        ...
        def maximize_max_hours(model):
            return sum(ref_total_hours(model, r) for r in model.R)
        ...
        def objective_function(model):
            return (
                ...
                maximize_max_hours(model) # non-weighted, maximize total hours first
            )
        ...
        try:
            ...
            if solver_name == "gurobi":
                ...
                solver.options["seconds"]    = time_limit_s      # time limit in seconds
                solver.options["ratioGap"]   = rel_gap           # relative MIP gap (e.g., 0.03 for 3%)
                if threads is not None:
